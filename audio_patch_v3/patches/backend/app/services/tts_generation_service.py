...
class TTSGenerationService:
    def generate(self, *, text: str, voice_id: str, output_dir: str = "artifacts/audio") -> dict:
        ...
        raw = ElevenLabsRealProvider().text_to_speech(text=text, voice_id=voice_id)
        if not raw.audio_bytes:
            raise RuntimeError("provider_returned_empty_audio")
        out_dir = Path(output_dir)
        out_dir.mkdir(parents=True, exist_ok=True)
        mp3_path = out_dir / f"{uuid4()}.mp3"
        mp3_path.write_bytes(raw.audio_bytes)
        ...
        return {
            ...
            "cost_estimate": estimate_tts_cost(raw.provider, text).dict(),
