...
def test_conversation_worker_creates_and_persists_contract_artifacts(db_session, tmp_path, monkeypatch):
    monkeypatch.setenv("ARTIFACT_ROOT", str(tmp_path))
    ...
    job = repo.create(
        ...
        request_json={"text": "hello from conversation", "voice": "default"},
    )

    result = process_conversation_job.run(str(job.id))
